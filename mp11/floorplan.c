#include "floorplan.h"
/* Name: Daniel Park || NETID: danielp7 || Date: 11/20/19
Intro: I approched this project with an understanding of trees. I read over the powerpoint and I made sure that I understood each step.
I then read over the floorplan.h file to make sure I knew what was inside each structure and what I should be using. The MP11 page and
the presentation had a lot of information on it and it helped with understanding the problems. I followed the algorithms and tried to 
solve all of the methods.
...
int is_leaf_node(node_t* ptr) {
  ...
  //if it has no left or right child then it is a leaf node.
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  //if it has no module then it is an internal node
  if (ptr->module == NULL) {
    return 1;
  }
  return 0;
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  //if a is null then b cannot reside in a.
  if (a == NULL) {
    return 0;
  }
  //if be is null, a's child is b, or if a = b then return 1;
  if (b == NULL || a->left == b || a->right == b || a == b) {
    return 1;
  }
  //traverse the whole tree checking
...
void rotate(node_t* ptr) {
  // TODO: 
  int temp;
  //set temp to w, then just swap h and w
  temp = ptr->module->w;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  //if the cutline is H, make it V, if V, make it H
  if (ptr->cutline == H) {
    ptr->cutline = V;
  } else {
    ptr->cutline = H;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  temp2 = a->parent;
  temp3 = b->parent;
  //if b parent left is b, make it a & vice versa
  if (temp->left == b) {
    temp->left = a;
  }
  if (temp->right == b) {
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  //recursive call the method for the left and right node
  postfix_traversal(ptr->left, nth, expression);
  postfix_traversal(ptr->right, nth, expression);
  //set each  expression's module to the pointer's module
  expression[*nth].module = ptr->module;
  //if module isn't null go to the next array space
  if (ptr->module != NULL) {
    *nth = *nth + 1;
    return;
  }
  //if the module is null then go to the next arrayspace cutline.
...
int get_total_resource(node_t* ptr)
{
  ...
  //if the module isn't null, get the value from inside and add it to sum
  if (ptr->module != NULL) {
